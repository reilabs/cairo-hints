...
use cairo_vm::{vm::{errors::vm_errors::VirtualMachineError, runners::cairo_runner::ResourceTracker}, felt::Felt252};
...
use cairo_vm::hint_processor::{cairo_1_hint_processor::{hint_processor::Cairo1HintProcessor}, hint_processor_definition::{HintProcessorLogic, HintReference}};
...
impl RpcHintProcessor {
    pub fn new(inner_processor: Cairo1HintProcessor, server: Option<String>) -> Self {
        Self { state: OracleState::Sending(Value::Null), path: Default::default(), inner_processor, server }
    }
